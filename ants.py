...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        if self.backwards and self.counter > 0 and self.counter < 3:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        counter = 0
        while bee_place != beehive:
            check_bees = random_or_none(bee_place.bees)
            if check_bees and counter >= min_range and counter <= max_range:
                return check_bees
            else:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        if self.digesting > 0:
            ...
            check_bee = random_or_none(self.place.bees)
            HungryAnt.eat_bee(self, check_bee)
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        if not other.is_container or self.contained_ant is None:
            return True
        return False
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.instance = QueenAnt.instance + 1
    ...
    def action(self, colony):
        ...
        if self.instance == 0:
            ...
            while place:
                ant = place.ant
                if ant not in self.ants and ant != None:
                    ant.damage = ant.damage * 2
                    self.ants.append(ant)
                if ant != None and ant.is_container and ant.contained_ant != None:
                    if ant.contained_ant not in self.ants :
                        ant.contained_ant.damage = ant.contained_ant.damage * 2
                        self.ants.append(ant.contained_ant)
                place = place.exit
        else:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while place != beehive:
            ant = place.ant
            if ant != self:
                if ant != None and ant.is_container and ant.contained_ant != self:
                    dict_insects[ant] = distane_place
                    dict_insects[ant.contained_ant] = distane_place
                elif ant != None and ant.is_container and ant.contained_ant == self:
                    dict_insects[ant] = distane_place
                elif ant != None:
                    dict_insects[ant] = distane_place
            bees = list(place.bees)
            for bee in bees:
                dict_insects[bee] = distane_place
            distane_place += 1
    ...
    def calculate_damage(self, distance):
        ...
        if damage <= 0:
            damage = 0
            return damage
        damage = damage - (distance * 0.2)
        damage = damage - (self.insects_shot * 0.05)
        return damage
